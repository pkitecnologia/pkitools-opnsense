...
ZABBIX_CONF_DIR="/usr/local/etc/zabbix_agentd.conf.d"
...
# Function to generate parameter string for Zabbix UserParameter
generate_param_string() {
    local num_params="$1"
    local param_string=""
    local i=1
...
# Function to generate Zabbix configuration file
generate_zabbix_config() {
    local manifest_file="$1"
    
    log "Generating Zabbix configuration"
    
    # Create Zabbix config directory if it doesn't exist
    if ! mkdir -p "$ZABBIX_CONF_DIR" 2>/dev/null; then
        log "WARNING: Failed to create Zabbix config directory: $ZABBIX_CONF_DIR"
        return 1
    fi
    
    # Generate configuration header
    {
        echo "# PKITools Zabbix UserParameters"
        echo "# Generated: $(date)"
        echo "# This file is automatically generated by PKITools-Monitor"
        echo ""
    } > "$ZABBIX_PKITOOLS_CONF"
    
    # Process manifest and generate UserParameters
    while IFS=' ' read -r script_filename zabbix_item_name num_params || [ -n "$script_filename" ]; do
        # Skip empty lines and comments
        case "$script_filename" in
            ''|'#'*) continue ;;
        esac
        
        # Default to 0 parameters if not specified or invalid
        if [ -z "$num_params" ] || ! echo "$num_params" | grep -q '^[0-9]\+$'; then
            num_params=0
        fi
        
        log "Generating Zabbix UserParameter: $zabbix_item_name ($num_params params)"
        
        # Generate UserParameter line
        if [ "$num_params" -gt 0 ]; then
            # With parameters
            param_string=$(generate_param_string "$num_params")
            echo "UserParameter=$zabbix_item_name[*],$SCRIPTS_DIR/$script_filename $param_string" >> "$ZABBIX_PKITOOLS_CONF"
        else
            # Without parameters
            echo "UserParameter=$zabbix_item_name,$SCRIPTS_DIR/$script_filename" >> "$ZABBIX_PKITOOLS_CONF"
        fi
        
    done < "$manifest_file"
    
    log "Zabbix configuration generated: $ZABBIX_PKITOOLS_CONF"
    return 0
}

# Function to reload Zabbix agent configuration
...
# Main execution starts here
{
    ...
    while IFS=' ' read -r script_filename zabbix_item_name num_params || [ -n "$script_filename" ]; do
        ...
        # Default to 0 parameters if not specified
        if [ -z "$num_params" ] || ! echo "$num_params" | grep -q '^[0-9]\+$'; then
            num_params=0
        fi
        ...
        log "Processing: $script_filename -> $zabbix_item_name ($num_params params)"
    ...
    if generate_zabbix_config "$TEMP_MANIFEST"; then
        log "SUCCESS: Zabbix configuration generated"
        
        # Reload Zabbix agent configuration
        reload_zabbix_config
    else
        log "WARNING: Failed to generate Zabbix configuration"
    fi
